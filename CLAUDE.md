...
| `contact` | **Contact-form inbox — the admin half only, by decision** (#63). `contact_messages`: UUIDv7 id so reverse-chron *is* `-id`, bare `tour_operator_id`, **required name** (nullable by mistake until contact/V3 — an inbox a person reads and replies to is worse off with an address and no name), email, summary, verbatim TEXT content. The row is immutable: read-state was dropped in #130 and `ContactMessageRepository.save` went with it. 3 endpoints — cursor list, get, delete (ADMIN+, audited with the **summary only**, so the sender's email stays out of the trail). **Intake is deleted and the inbox is a mailbox nothing can fill** — but the storefront's `/contact` page now describes what it accepts, through `StorefrontContactQuery`, which is the parked domain's **first reader** and the first thing to make its survival pay. `SubmitContactMessageUseCase` was the only creator; `ContactMessage.submit` and the four validation value objects are **parked deliberately rather than deleted**, because the intake returns and the domain is what makes that cheap. What it established stays true for then: throttled per tenant 30/h **before** validation, so rejected input costs a token too, and **not audited by decision** — the row is its own record, so a `STOREFRONT` audit actor is still unneeded. | Built + merged |
...
`storefront` resolves the tenant from the host and owns **thirteen addresses**. Twelve are pages, and **all twelve serve real data** — six routes, each with and without a locale prefix: `/`, `/experiences`, `/experiences/{handle}`, `/policies/{type}`, `/pages/{handle}`, `/contact`. The thirteenth is **`/password`**, the gate — `GET` renders it, `POST` submits it, and it is a route like any other, which is why `PasswordPageController` is one of the six controllers below.

There are **seven controllers**: `StorefrontHomeController`, `StorefrontExperienceListController`, `StorefrontExperienceDetailController`, `StorefrontCmsPageController`, `StorefrontPolicyController`, `StorefrontContactController`, `PasswordPageController`. **Each page route names its own `pageType` and canonical path** at the `StorefrontGlobalsResponse` factory it calls — `index`, `list-experiences`, `experience`, `page`, `policy`, `contact` — rather than having them inferred from which objects are present, because a route whose object is absent would otherwise become the index by accident. The experiences listing is the live instance of that hazard: its body is byte-identical to the home page's apart from those two fields. **Every factory is named; there is no public `from`** (#203), and `SwitcherUrlsRoundTripThroughLocaleRuleTest` reflects over the class so a new one that mints switcher urls fails the build until it is covered — it caught the policy factory. The tenant seam is `TenantHandleResolver` plus `StorefrontTourOperatorQuery`. There is no `StorefrontTenantQuery` — it was never rebuilt under that name. The password gate is back in full.

**`/contact` is a route of ours, not of Shopify's** — they assign a `page.contact` template to an ordinary CMS page and only the *submission* goes to `/contact`. Chosen deliberately (`OPEN-WORK.md`), and the live consequence is that an operator can also author a CMS page at `/pages/contact`; the seeded one has. It serves `contactForm` — the four fields the inbox accepts with their limits, read off `contact`'s domain value objects through `StorefrontContactQuery`, **the first reader of the domain parked when intake was deleted**. No `action`: there is nowhere to post yet, and naming an address nothing serves would make the contract lie.
...
**An unmatched path on a tenant host is the storefront's own 404, not a 401.** A `PublicRoute` list is an allowlist, so *every* way of missing it fails closed — and on a public page failing closed is the wrong answer. Every storefront route constrains its path variables (a security decision, kept), so a visitor's typo — `/policies/legal_notice`, `/pages/About_Us` — matched no route, never reached MVC, and was told `401 Authentication required` by a site with no login.

`UnauthenticatedRequestPolicy` is the seam: a third `shared.web.security` SPI beside `PublicRouteRegistrar` and `RateLimitRuleRegistrar`, and an SPI because it must be — `shared` may not import a context, so the entry point cannot ask the storefront a question except through an interface the kernel declares. `StorefrontUnauthenticatedRequests` claims **the host, not a list of prefixes**: on a host that resolves to a tenant every path is the storefront's, so nothing has to be kept in step with the routes. **It does not widen the allowlist by one byte** — `authorizeHttpRequests` is untouched and only the response to a request that already lost is reshaped.

Two carve-outs, both deliberate: **`/api/**` keeps its 401** even on a tenant host, because the dev stack puts both surfaces on one origin and an expired token answering "There is no storefront at this address" sends the reader hunting a routing bug that is not there; and **the apex host is untouched**, since it serves the admin API where a 401 is the honest answer.

**Every implementation of one of these three SPIs must be a `@Component`**, and ArchUnit enforces it (`web_security_spi_implementations_are_components`). Spring injects an **empty list** when nothing implements an SPI — no error — so a dropped annotation silently removes the rule. A slice test cannot catch it either: `@Import(Foo.class)` registers the bean regardless of the annotation, measured green with the production behaviour gone.
...
`servesHeadAsWellAsGet` pins it across **all thirteen** addresses — the entry is per route as well as per method, so coverage is per route too. Registration is not where the risk is: `StorefrontPublicRoutes` loops `PAGE_ROUTES` adding GET+HEAD per route, so a new page route is registered on both methods by construction. **The gate is the exception and the one to watch** — `/password` needs GET, HEAD *and* POST, listed by hand rather than looped, which is exactly where an entry goes missing. Dropping its HEAD line fails `PasswordPageControllerTest.servesHeadAsWellAsGet` with a 401; verified by deleting it.
