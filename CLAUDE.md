...
**The storefront is the second surface, and its auth decision is "none yet."** `storefront` server-renders the public site in-process (MAP's render-path decision, in Decided) and registers its eight routes — `/`, `/{locale}`, `/experiences`, `/{locale}/experiences`, `/policies/{type}`, `/{locale}/policies/{type}`, `GET`+`POST /password` — through the same `PublicRouteRegistrar` every other public route uses, not a host-matched `SecurityFilterChain`. That is where it ends up, but nothing today needs storefront requests to have *different* security, only *no* authentication: `JwtAuthenticationFilter` ignores anonymous requests and `ApiRateLimitFilter` passes through anything without a JWT principal. Per-IP limits still go through `RateLimitRuleRegistrar` like everyone else's (`POST /password`, 20/h). Revisit when the storefront needs its own CORS or error shape.

**What gates the storefront is a `HandlerInterceptor`, not the security chain.** The password gate is a per-tenant, per-request decision on data, so it sits in `storefront/infrastructure/web` on **exactly the storefront's page routes** — `/`, the locale template and both the `/experiences` and `/policies/{type}` pairs, **never `/**`** (which would front `/api/**`) and no longer `/*` (which was wider than the storefront). A page route added later has to be added there too, and nothing fails if you forget: the page simply serves on a locked store. It runs **before** locale resolution, and that ordering is the load-bearing part: gate second and a locked store 404s an unpublished locale while redirecting a published one, which tells an anonymous visitor the store exists and which locales it has. `StorefrontHomeControllerTest.aLockedStoreRedirectsEvenForALocaleItDoesNotPublish` is that ordering; nothing else pins it.
