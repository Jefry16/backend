...
- **Use cases are plain POJOs** — no Spring annotations — hand-wired as `@Bean`s in each context's `infrastructure/config/<Ctx>UseCaseConfig`. **ArchUnit enforces an allowlist**: the application layer may depend on our own code, the JDK and SLF4J, and **nothing else, with no exemptions** — an allowlist rather than a list of banned frameworks, because the latter only catches the ones someone thought to name. Need a library in a use case? That is the signal to write a port.
