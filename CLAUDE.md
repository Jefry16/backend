...
**The storefront is a placeholder, and its auth decision is still "none yet."** It was cut back on 2026-08-11 (`e379cf2`): `storefront` still resolves the tenant from the host and still owns its six page addresses — `/`, `/{locale}`, `/experiences`, `/{locale}/experiences`, `/policies/{type}`, `/{locale}/policies/{type}` — but every one of them renders the same "coming soon" page, and an unknown handle is a 404. **Everything that read a storefront's data is deleted**: the shop/brand/policy/experience queries (`StorefrontShopQuery`, `StorefrontExperienceQuery`), the theme object model, all six page use cases, every view and template but two, and the password gate. What is left is `TenantHandleResolver` + `StorefrontTenantQuery.exists(handle)` + one controller. The design record survives in PATTERNS §2a (marked as a specification, not a description) and in git; rebuild from those rather than from memory. Routes stay unauthenticated through the same `PublicRouteRegistrar` every other public route uses, not a host-matched `SecurityFilterChain` — nothing needs storefront requests to have *different* security, only *no* authentication.

**The password gate is gone with the data, and that is a decision, not an oversight.** A page that renders no operator data has nothing to protect. The admin endpoints under `/api/tour-operators/{id}/storefront-password` still store a password and **nothing enforces it** (the controller's Javadoc says so). When the real pages return, so does the gate — and with it the rule that cost the most to learn: **it must run before locale resolution**, or a locked store 404s an unpublished locale while redirecting a published one, telling an anonymous visitor the store exists and which locales it has. That was pinned by `StorefrontHomeControllerTest.aLockedStoreRedirectsEvenForALocaleItDoesNotPublish`; recover it from git alongside the gate.
