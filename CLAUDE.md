...
- **Use cases are plain POJOs** — no Spring annotations — hand-wired as `@Bean`s in each context's `infrastructure/config/<Ctx>UseCaseConfig`. **ArchUnit enforces an allowlist**: the application layer may depend on our own code, the JDK and SLF4J, and nothing else — an allowlist rather than a list of banned frameworks, because the latter only catches the ones someone thought to name. One class is frozen as a named exception (`MetafieldValueValidator`, which holds a Jackson `ObjectMapper`; debt, wants a parser port). A second class cannot join without editing that list.
