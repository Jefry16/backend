...
**The storefront serves its globals as JSON, and nothing about it is authenticated — a locked store is gated by the storefront password, which is an interceptor and not a login.** It was cut back to a placeholder on 2026-08-11 (`e379cf2`) and the home page came back the same day. `storefront` resolves the tenant from the host and owns eight addresses — `/`, `/{locale}`, `/experiences`, `/{locale}/experiences`, `/policies/{type}`, `/{locale}/policies/{type}`, `/pages/{handle}`, `/{locale}/pages/{handle}`. **`/` and `/{locale}` serve the real contract**: `tourOperator` — Shopify's `shop`, renamed 2026-08-13 (brand, palette, social links, policies, currency, timezone, **a structured `address`** with the country resolved, **metafields** — Shopify's `shop.metafields.<namespace>.<key>` shape with our type codes), `localization`, `routes`, `linklists` (menus by handle, links resolved and dead ones pruned), `featuredExperiences` (published + featured, **capped at 12**, top-level like Shopify's catalogue) and `pageTitle`/`pageDescription`/`ogImageUrl`/`canonicalUrl`/`pageType` — the same set Shopify's index template gets, since that template has no object of its own. **`/pages/{handle}` is the second real route** — globals plus a `page` object (Shopify's, so `pageTitle` carries the title tag and `page` is the CMS record). The other four still answer `{"handle","status"}`. An unknown handle, and a locale the operator does not publish, are both a 404 in the application's usual error shape — deliberately indistinguishable. **JSON and not HTML until the contract is settled**: a wrong field is visible in a body and invisible under markup nobody reads yet. **The 2026-08-11 cutback deleted every read path** — the operator/brand/policy/experience queries, the theme object model, all six page use cases, every view and every template, and the password gate — **and most of it has since been rebuilt**, so treat that deletion as history rather than as the current shape. Today there are **four controllers** (`StorefrontHomeController`, `StorefrontCmsPageController`, `StorefrontPlaceholderController`, `PasswordPageController`), the tenant seam is `TenantHandleResolver` + `StorefrontTourOperatorQuery` (there is no `StorefrontTenantQuery`; it was never rebuilt under that name), and the password gate is back in full. **It answers JSON, not HTML, on purpose**: the point of the placeholder period is getting the *data* right before themes exist, and markup nobody reads hides a wrong field. Mustache itself stays — the dependency and `StorefrontMustacheConfig`'s compiler are the render-path decision (PATTERNS §2b) waiting for the themes, and its settings test is what keeps the traps in `STACK.md` true meanwhile. **The storefront's record lives in `PATTERNS.md` — §2a for the contract, §2b for the render path — and nowhere else.** MAP carries a pointer, not a copy (moved 2026-08-15); rebuild from those rather than from memory. Routes stay unauthenticated through the same `PublicRouteRegistrar` every other public route uses, not a host-matched `SecurityFilterChain` — nothing needs storefront requests to have *different* security, only *no* authentication.
