...
/**
 * Per-locale overlays for one CMS page's metafields. Same flow as its two twins.
 */
@WebMvcTest(PageMetafieldTranslationController.class)
@ExtendWith(RestDocumentationExtension.class)
@Import({SecurityConfig.class, WebConfig.class, ListQueryParser.class})
class PageMetafieldTranslationControllerDocumentationTest {

    private static final String OP = "019f7f33-1833-7dc1-b008-47e6c68b3ea2";
    private static final String PAGE_ID = "019f7f33-1833-7dc1-b008-47e6c68b3eb5";
    ...
    void listLocales() throws Exception {
        when(listLocalesUseCase.execute(any(), any(), eq(MetafieldOwnerType.PAGE), any()))
                .thenReturn(List.of("en", "fr"));

        mockMvc.perform(get("/api/tour-operators/{id}/pages/{pageId}/metafield-translations", OP, PAGE_ID)
    ...
    void getOne() throws Exception {
        when(getUseCase.execute(any(), any(), eq(MetafieldOwnerType.PAGE), any(), anyString()))
                .thenReturn(Map.of("custom.opening-hours", "Mon-Fri 09:00-18:00"));

        mockMvc.perform(get("/api/tour-operators/{id}/pages/{pageId}/metafield-translations/{locale}", OP, PAGE_ID, "en")
                        .header("Authorization", BEARER))
                .andExpect(status().isOk())
    ...
    void upsert() throws Exception {
        mockMvc.perform(put("/api/tour-operators/{id}/pages/{pageId}/metafield-translations/{locale}", OP, PAGE_ID, "en")
                        .header("Authorization", BEARER)
    ...
    void deleteLocale() throws Exception {
        mockMvc.perform(delete("/api/tour-operators/{id}/pages/{pageId}/metafield-translations/{locale}", OP, PAGE_ID, "en")
                        .header("Authorization", BEARER))
                .andExpect(status().isNoContent())
