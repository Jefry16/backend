 ...
 * locales — the gate exists to withhold exactly that, and a partial {@code shop}
 ...
 */
public record PasswordPageResponse(String shopName, String passwordMessage, boolean error) {

    public static PasswordPageResponse from(PasswordPageOutput output, boolean error) {
        return new PasswordPageResponse(output.shopName(), output.passwordMessage(), error);
    }
}
