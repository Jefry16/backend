...
/**
 * The copy the gate shows: the shop's name and the operator's message for
 ...
 *
 * <p>It reads the gate and nothing else. The shop's name is not translated (V8
...
public class GetPasswordPageUseCase {
    ...
    public Optional<PasswordPageOutput> execute(String handle) {
        return storefrontShopQuery.findGate(handle)
                .map(gate -> new PasswordPageOutput(gate.shopName(), gate.passwordMessage()));
    }
}
