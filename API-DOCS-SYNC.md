...
not left until later: its surface is public, unauthenticated and rendered rather
than called, so it answers to different rules than the admin API and there is no
point pinning a contract that is waiting for themes. Its contract is written down
in `PATTERNS.md` §2a instead. The other eleven are done.
